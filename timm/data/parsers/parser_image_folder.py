...
class ParserDental(Parser):
    ...
    def __init__(
        ...
        for i in self.coco.anns:
            ...
            #category id map to target 这里分2类，caries,others
            if self.coco.anns[i]['category_id']==2:
                row.append(1)
            else:
                row.append(0)
...
class ParserDentalSeg(ParserDental):
    ...
    def __init__(
        ...
        for i in self.coco.anns:
            ...
            self.samples_dic[row[1]].append(row)
    ...
    def balance_samples(self):
        ...
        target_num = len(self.samples_dic[1])
        
        print(target_num)

        self.samples = self.samples_dic[1]+random.sample(self.samples_dic[0],target_num)
